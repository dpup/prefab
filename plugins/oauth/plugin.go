...
func (b *Builder) Build() *OAuthPlugin {
	...
	// Use user-provided store or default in-memory store
	var store ClientStore
	if p.userClientStore != nil {
		store = p.userClientStore
	} else {
		store = newMemoryClientStore()
	}

	// Create adapters that wrap the store
	p.clientStore = newClientStoreAdapter(store)
	p.tokenStore = newTokenStoreAdapter()
